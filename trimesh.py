...
class Trimesh():
    ...
    def manual_subdivide(self, new_vertices, new_faces, project_to_sphere = True, modified = True):
        ...
        num_new_vertices = new_vertices.shape[0] - self.vertices.shape[0]
        
        if self.level==0:
            eye = self.filters[0]
            
            P = np.vstack((eye,np.zeros((num_new_vertices,eye.shape[1]))))
            ...
            P = np.vstack((np.identity(PQ.shape[1]),np.zeros((num_new_vertices,PQ.shape[1]))))
